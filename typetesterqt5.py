...
from PyQt5.QtWidgets import (QWidget, QGridLayout, QLineEdit, QTextEdit,
...
class Example(QWidget):
    ...
    def initUI(self):
        ...
        self.entry_widget.textChanged.connect(lambda changed:
                                              self.text_widget_handler(
                                               changed, self.random_line))
    ...
    def matcher(self, string_a, string_b):
        ...
        if string_a == string_b[0:len(string_a)]:
            ...
            for i, char in enumerate(
                            string_a[self.match_end_index+1:len(string_a)]):
                if string_a[0:self.match_end_index+1+i] == string_b[
                            0:self.match_end_index+1+i]:
                    matched += 1
    ...
    def text_widget_handler(self, string_a, string_b):
        ...
        if not_matched >= 1:
            ...
            self.text_widget.setHtml("<font color=\"Gray\">" +
                                     string_b[:matched] + "</font>" +
                                     # self.next_char +
                                     # string_b[matched] + "</p1>" +
                                     "<font color=\"Red\">" +
            ...
            self.text_widget.setHtml("<font color=\"Gray\">" +
                                     string_b[:matched] + "</font>" +
                                     self.next_char + string_b[matched] +
                                     "</p1>" +
                                     # "<font color=\"Red\">" +
                                     # string_b[matched:matched+not_matched] +
                                     # "</font>" +
